...
//20220404 add
__global__ void forward_avgpool_depth_layer_kernel(int n, int w, int h, int c, int out_c, int batch, float *input, float *output, int *indexes)
{
	int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	if (id >= n) return;
...
//20220404 add
__global__ void backward_avgpool_depth_layer_kernel(int n, int w, int h, int c, int batch, float *delta, float *prev_delta, int *indexes)
{
	int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	if (id >= n) return;
	...
	for (i = 0; i < c; ++i) {
		...
		prev_delta[in_index] += delta[out_index] / c;
	}
}
...
extern "C" void forward_local_avgpool_layer_gpu(maxpool_layer layer, network_state state)
{
	...
	if (layer.maxpool_depth) {
		...
		forward_avgpool_depth_layer_kernel << <cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >> >(
			n, layer.w, layer.h, layer.c, layer.out_c, layer.batch, state.input, layer.output_gpu, layer.indexes_gpu);
		CHECK_CUDA(cudaPeekAtLastError());
...
extern "C" void backward_local_avgpool_layer_gpu(maxpool_layer layer, network_state state)
{
	...
	if (layer.maxpool_depth) {
		...
		backward_avgpool_depth_layer_kernel << <cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >> >(n, layer.w, layer.h, layer.c, layer.batch, layer.delta_gpu, state.delta, layer.indexes_gpu);
		CHECK_CUDA(cudaPeekAtLastError());
		return;
	}
